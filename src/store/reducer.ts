import { GameAction, CountryId, Player } from "../types/game";
import { GameStore, RoundSummary } from "./state";
...
function handleStartGame(state: GameStore, payload: { playerIds: string[] }): Partial<GameStore> {
...
function handleDealRound(state: GameStore, payload: { round: number }): Partial<GameStore> {
    const { round } = payload;
...
function handleResolveRound(state: GameStore): Partial<GameStore> {
...
function handlePlaceToken(state: GameStore, payload: { playerId: string; countryId: CountryId }): Partial<GameStore> {
    const { playerId, countryId } = payload;
...
export function reducer(state: GameStore, action: GameAction): Partial<GameStore> {
    switch (action.type) {
        case "ENTER_SETUP":
        ...
        case "SET_HIGHLIGHTED_PLAYER":
            return { ...state, highlightedPlayerId: action.payload };

        case "START_GAME":
            return handleStartGame(state, action.payload);

        case "DEAL_ROUND":
            return handleDealRound(state, action.payload);

        case "RESOLVE_ROUND":
            return handleResolveRound(state);
